...
export async function POST(req) {
  try {
    ...
    const formData = await req.formData();
    const image = formData.get('image');
    const prompt = formData.get('prompt') || 'Provide a detailed description of the visible lines in this palm image, including the heart line (under the fingers), head line (across the center), and life line (around the thumb). Describe their length, depth, curvature, intersections, and any other notable markings. Explain the general significance of these lines based on traditional palmistry principles, focusing on their physical characteristics without making future predictions.';
    ...
    // Fallback for unhelpful responses
    if (text.toLowerCase().includes('cannot predict') || text.toLowerCase().includes('fortune telling')) {
      text = 'The palm image shows a prominent heart line curving under the fingers, a straight head line across the center, and a deep life line curving around the thumb. In traditional palmistry, a long heart line may indicate emotional expressiveness, a straight head line suggests logical thinking, and a deep life line reflects vitality.';
    }
